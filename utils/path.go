...
func Path(rel string) string {
	...
	//if is a path to a folder, return the absolute path to the folder
	if string(rel[len(rel)-1]) == "/" || string(rel[len(rel)-1]) == "\\" {
		return filepath.Join(basepath, rel) + string(filepath.Separator)
	}
